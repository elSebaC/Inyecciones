...
import EditInjection from "./EditInjection";
import { Card } from "./ui";
...
export default function HistoryTab({
  ...
  onUpdate: (id: string, patch: Partial<NewInjection>) => Promise<void>;
}) {
  ...
  if (!injections.length) {
    return (
      <Card>
        <p className="text-sm text-stone-500">Aún no hay inyecciones registradas.</p>
      </Card>
    );
  }
                    ...
                    <div key={i.id} className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex shrink-0 gap-1">
                        <button
                          ...
                          className="rounded-lg px-2 py-1 text-xs font-medium text-[#2f7fc1] hover:bg-blue-50"
                        >
                          Editar
                        </button>
                        <button
